...
//
//   let allCarsByCity = try? newJSONDecoder().decode(AllCarsByCity.self, from: jsonData)

import Foundation

// MARK: - AllCarsByCityElement
struct AllCarsByCityElement: Codable {
    let id, idOwner: Int
    let owner: String
    let image: String
    ...
    let availableTimeFrom, availableTimeTo: String
...
typealias AllCarsByCity = [AllCarsByCityElement]
