        ...
        DispatchQueue.global().async { [weak self] in
            APIClient.agrre_request(id_request: id) { (Result) in
                ...
                case .success(let response):
                    print(response)
                    if response == "True" {
                        Alert.show("Done".localized, massege: "Thank You ,request accepted successfully".localized, context: self!)
                        self?.getData()
                    }
                case .failure(let error):
                    print(error.localizedDescription)
    ...
    func refuseRequest(id: Int) {
        DispatchQueue.global().async { [weak self] in
            APIClient.refuseRequest(id_request: id) { (Result) in
                ...
                case .success(let response):
                    print(response)
                    Alert.show("Deleted".localized, massege:"Request Refused Successfully".localized, context: self!)
                    self?.getData()
                case .failure(let error):
                    print(error.localizedDescription)
                }
